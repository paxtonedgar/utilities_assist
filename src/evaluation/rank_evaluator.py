...
class RankEvaluator:
    ...
    def evaluate_search_strategies(
        ...
        logger.info(
            f"Evaluating {len(strategies)} strategies on dataset '{dataset.name}' "
        ...
        for strategy in strategies:
            ...
            try:
                result = self.evaluation_client.evaluate_search_quality(
                    queries=dataset.queries, index=index, search_strategy=strategy, k=k
                )
                ...
                logger.info(
                    f"{strategy} results: nDCG@10={metrics.get('avg_ndcg_at_10', 0):.3f}, "
    ...
    def compare_strategies(
        self, results: Dict[str, EvaluationResult], output_path: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        for metric in metrics_to_compare:
            ...
            comparison["best_strategy"][metric] = (
                sorted_strategies[0][0] if sorted_strategies else None
            )
        ...
        if len(results) >= 2:
            ...
            for strategy in strategies[1:]:
                ...
                for metric in metrics_to_compare:
                    ...
                    if baseline_value > 0:
                        improvement = (
                            (strategy_value - baseline_value) / baseline_value
                        ) * 100
    ...
    def save_dataset(self, dataset: EvaluationDataset, output_path: str) -> None:
    ...
    def run_continuous_evaluation(
        ...
        for strategy, result in results.items():
            ...
            if ndcg < baseline_threshold:
                alerts.append(
                    f"⚠️  {strategy}: nDCG@10 ({ndcg:.3f}) below threshold ({baseline_threshold})"
                )

            if recall < baseline_threshold:
                alerts.append(
                    f"⚠️  {strategy}: Recall@50 ({recall:.3f}) below threshold ({baseline_threshold})"
                )
        ...
        summary = {
            ...
            "results_summary": {
                strategy: {
                    ...
                    "precision_at_10": result.overall_metrics.get(
