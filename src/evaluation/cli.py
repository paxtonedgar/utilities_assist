...
def run_evaluation(
    ...
    try:
        ...
        if len(results) > 1:
            best_ndcg = max(results.items(), key=lambda x: x[1].overall_metrics.get('avg_ndcg_at_10', 0))
            best_recall = max(results.items(), key=lambda x: x[1].overall_metrics.get('avg_recall_at_50', 0))
            
            print(f"\nBEST PERFORMERS:")
            print(f"  nDCG@10:  {best_ndcg[0]} ({best_ndcg[1].overall_metrics.get('avg_ndcg_at_10', 0):.3f})")
            print(f"  Recall@50: {best_recall[0]} ({best_recall[1].overall_metrics.get('avg_recall_at_50', 0):.3f})")
...
def create_dataset_from_logs(
    ...
    try:
        ...
        logger.info(f"Dataset created with {len(dataset.queries)} queries and saved to {output_path}")
...
"""
    ...
    sample_parser = subparsers.add_parser('create-sample', help='Create sample evaluation dataset')
    sample_parser.add_argument('--output', '-o', required=True, help='Output path for dataset')
    sample_parser.add_argument('--queries', '-q', type=int, default=10, help='Number of sample queries')
    
    # Evaluate command
    eval_parser = subparsers.add_parser('evaluate', help='Run evaluation on search strategies')
    eval_parser.add_argument('--dataset', '-d', required=True, help='Path to evaluation dataset')
    eval_parser.add_argument('--strategies', '-s', nargs='+', default=['enhanced_rrf', 'bm25'],
                           help='Search strategies to evaluate')
    eval_parser.add_argument('--index', '-i', help='OpenSearch index to evaluate against')
    eval_parser.add_argument('--k', type=int, default=50, help='Number of results to retrieve')
    eval_parser.add_argument('--output-dir', default='evaluation_results', help='Output directory for results')
    
    # Continuous evaluation command
    continuous_parser = subparsers.add_parser('continuous', help='Run continuous evaluation')
    continuous_parser.add_argument('--dataset', '-d', required=True, help='Path to evaluation dataset')
    continuous_parser.add_argument('--strategies', '-s', nargs='+', default=['enhanced_rrf'],
                                 help='Search strategies to evaluate')
    continuous_parser.add_argument('--output-dir', default='evaluation_results', help='Output directory for results')
    continuous_parser.add_argument('--threshold', '-t', type=float, default=0.8,
                                 help='Performance threshold for alerts')
    
    # Create dataset from logs command
    logs_parser = subparsers.add_parser('from-logs', help='Create dataset from search logs')
    logs_parser.add_argument('--log-file', '-l', required=True, help='Path to search log file')
    logs_parser.add_argument('--output', '-o', required=True, help='Output path for dataset')
    logs_parser.add_argument('--max-queries', type=int, default=100, help='Maximum queries to process')
    logs_parser.add_argument('--threshold', type=float, default=0.7, help='Relevance threshold')
