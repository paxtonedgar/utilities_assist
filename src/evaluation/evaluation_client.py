...
class EvaluationQuery:
    ...
    query_id: str
    query_text: str
    relevant_docs: List[Dict[str, Any]]  # List of {"_id": doc_id, "rating": relevance_score}
...
class OpenSearchEvaluationClient:
    ...
    def evaluate_search_quality(
        ...
        try:
            ...
            logger.info(f"Evaluation completed: nDCG@10={overall_metrics.get('avg_ndcg_at_10', 0):.3f}, "
    ...
    def _execute_rank_evaluation(
        ...
        auth = None
        if hasattr(self.settings.opensearch, 'username') and self.settings.opensearch.username:
            auth = (self.settings.opensearch.username, self.settings.opensearch.password)
        ...
        if response.status_code != 200:
            raise Exception(f"Rank evaluation failed: {response.status_code} - {response.text}")
    ...
    def _parse_evaluation_response(
        ...
        for query in queries:
            ...
            ndcg_at_10 = query_detail.get("metric_details", {}).get("ndcg", {}).get("ndcg", metric_scores)
            recall_at_50 = query_detail.get("metric_details", {}).get("recall", {}).get("recall", 0.0)
            precision_at_10 = query_detail.get("metric_details", {}).get("precision", {}).get("precision", 0.0)
            mrr = query_detail.get("metric_details", {}).get("mean_reciprocal_rank", {}).get("mean_reciprocal_rank", 0.0)
    ...
    def _calculate_overall_metrics(
        ...
        return {
            ...
            "avg_recall_at_50": sum(m.recall_at_50 for m in query_metrics) / total_queries,
            "avg_precision_at_10": sum(m.precision_at_10 for m in query_metrics) / total_queries,
            "avg_mean_reciprocal_rank": sum(m.mean_reciprocal_rank for m in query_metrics) / total_queries,
