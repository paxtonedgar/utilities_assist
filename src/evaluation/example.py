...
from .metrics import calculate_metrics_summary, aggregate_metrics
...
def example_metrics_calculation():
    ...
    print(f"Query: 'OpenSearch configuration tutorial'")
    ...
    # Show interpretation
    print("\nInterpretation:")
    print(f"  • nDCG@10 = {metrics.get('ndcg_at_10', 0):.3f} - Measures ranking quality with position discount")
    print(f"  • Recall@10 = {metrics.get('recall_at_10', 0):.3f} - Fraction of relevant docs found in top 10")
    print(f"  • Precision@10 = {metrics.get('precision_at_10', 0):.3f} - Fraction of top 10 that are relevant")
    print(f"  • MRR = {metrics.get('mean_reciprocal_rank', 0):.3f} - Reciprocal rank of first relevant doc")
...
def example_strategy_comparison():
    ...
    for strategy, data in strategy_results.items():
        ...
        avg_precision = sum(m["precision_at_10"] for m in metrics_list) / len(metrics_list)
    ...
    enhanced_rrf_ndcg = sum(m["ndcg_at_10"] for m in strategy_results["enhanced_rrf"]["query_metrics"]) / 5
    bm25_ndcg = sum(m["ndcg_at_10"] for m in strategy_results["bm25"]["query_metrics"]) / 5
    
    improvement = ((enhanced_rrf_ndcg - bm25_ndcg) / bm25_ndcg) * 100
    
    print(f"\nIMPROVEMENT ANALYSIS:")
    print(f"  Enhanced RRF vs BM25:")
    ...
    if improvement > 5:
        ...
        print(f"  ❌ No improvement or degradation")
...
def example_dataset_creation_from_logs():
    ...
    print("   3. The system will auto-generate relevance judgments based on search scores")
