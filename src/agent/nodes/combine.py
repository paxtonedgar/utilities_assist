...
async def combine_node(state, config=None, *, store=None):
    ...
    try:
        ...
        logger.info(f"NODE_END combine | combined_results={len(combined_results)} | context_length={len(final_context)}")
        ...
        return from_state_dict(incoming_type, {**s, "error_messages": s.get("error_messages", []) + [str(e)]})
