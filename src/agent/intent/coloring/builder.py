...
class ColorsBuilder:
    ...
    def compute(self, text: str, features: Dict[str, int]) -> Colors:
        ...
        try:
            ...
            logger.debug(
                f"Colors computed: act={actionability_est:.1f}, suites={len(suite_affinity)}, spec={specificity}"
            )
    ...
    def _detect_troubleshooting(self, text: str) -> bool:
        ...
        return has_pattern_match(text, "error_codes") or has_pattern_match(
            text, "stack_trace"
        )
...
def get_colors_builder() -> ColorsBuilder:
    ...
    if _builder_instance is None:
        ...
        _builder_instance = ColorsBuilder(
            registry=registry, weights=weights, thresholds=thresholds
        )
