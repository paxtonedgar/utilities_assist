...
def example_profile_query():
    ...
    try:
        ...
        print(f"Total hits: {search_result.get('hits', {}).get('total', {}).get('value', 0)}")
        print(f"Max score: {search_result.get('hits', {}).get('max_score', 0)}")
        
        print(f"\nProfile Summary:")
        print(f"Total shards: {profile_data['summary']['total_shards']}")
        print(f"Average query time: {profile_data['summary']['avg_query_time_ms']:.2f}ms")
    ...
    try:
        ...
        for strategy, data in result['strategy_results'].items():
            ...
            if 'profile_analysis' in data:
                ...
                print(f"  Avg Query Time: {profile['summary']['avg_query_time_ms']:.2f}ms")
                print(f"  Slowest Component: {profile['summary']['slowest_component']}")
        
        # Show comparison summary
        comparison = result['comparison']
        print(f"\nComparison Summary:")
        print(f"Fastest strategy: {comparison['fastest_strategy']} ({comparison['performance_comparison']['fastest_time_ms']}ms)")
        print(f"Most relevant: {comparison['most_relevant_strategy']} (score: {comparison['relevance_comparison']['highest_max_score']:.4f})")
        print(f"Most hits: {comparison['most_comprehensive_strategy']} ({comparison['relevance_comparison']['most_hits']} hits)")
...
def example_query_analysis():
    ...
    try:
        ...
        for component in analysis.components:
            print(f"  - {component.type}: {component.description} (weight: {component.weight})")
...
if __name__ == "__main__":
    ...
    if results:
        ...
        print("   from src.debug import OpenSearchDebugClient, QueryAnalyzer, PerformanceMonitor")
