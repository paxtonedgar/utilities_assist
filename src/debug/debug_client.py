...
class OpenSearchDebugClient:
    ...
    def explain_query(
        ...
        try:
            ...
            response = self.client.explain(
                index=target_index, id=document_id, body=opensearch_query
            )

            logger.info(
                f"Explained query for document {document_id} in index {target_index}"
            )
            return response
    ...
    def analyze_slow_queries(
        self, queries: List[str], threshold_ms: int = 1000, index: Optional[str] = None
    ) -> Dict[str, Any]:
            ...
            try:
                ...
                if took_ms > threshold_ms:
                    slow_query_analysis = {
                        ...
                        "took_ms": took_ms,
                        "profile": profile_result.get("profile", {}),
                        "hits_total": profile_result.get("hits", {})
                        .get("total", {})
                        .get("value", 0),
                    }

                    # Analyze bottlenecks
                    bottlenecks = self._analyze_query_bottlenecks(
    ...
    def _build_query_for_strategy(
        self, query_text: str, strategy: str, filters: Optional[SearchFilters] = None
    ) -> Dict[str, Any]:
        ...
        if strategy == "enhanced_rrf":
            return self.opensearch_client._build_hybrid_query(
                query=query_text, k=50, filters=filters
            )
        elif strategy == "bm25":
    ...
    def _compare_strategies(self, strategies_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            Comparison analysis
        """
        comparison = {"performance": {}, "result_overlap": {}, "recommendations": []}
        ...
        if len(strategies_results) >= 2:
            ...
            for i, strategy1 in enumerate(strategy_names):
                for strategy2 in strategy_names[i + 1 :]:
                    if (
                        "error" not in strategies_results[strategy1]
                        and "error" not in strategies_results[strategy2]
                    ):
                        overlap = self._calculate_result_overlap(
                            strategies_results[strategy1]
                            .get("hits", {})
                            .get("hits", []),
                            strategies_results[strategy2]
                            .get("hits", {})
                            .get("hits", []),
                        )
                        comparison["result_overlap"][f"{strategy1}_vs_{strategy2}"] = (
                            overlap
                        )

        # Generate recommendations
        comparison["recommendations"] = self._generate_strategy_recommendations(
            comparison
        )
    ...
    def _calculate_result_overlap(
        self, hits1: List[Dict[str, Any]], hits2: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
            ...
            for search in searches:
                ...
                for query_component in query_info:
                    ...
                    if time_ms > 100:  # More than 100ms
                        bottlenecks.append(
                            f"Slow {query_type} query component: {time_ms:.1f}ms"
                        )
                    ...
                    if query_type == "TermQuery" and time_ms > 50:
                        ...
                        bottlenecks.append(
                            "Complex boolean query - consider simplification"
                        )
                    elif "wildcard" in query_type.lower() and time_ms > 100:
                        bottlenecks.append(
                            "Expensive wildcard query - consider alternatives"
                        )
    ...
    def _generate_optimization_suggestions(
        ...
        if "wildcard" in str(common_bottlenecks).lower():
            suggestions.append(
                "Reduce wildcard query usage - use prefix queries or filters instead"
            )

        if "boolean" in str(common_bottlenecks).lower():
            suggestions.append(
                "Simplify complex boolean queries - break into multiple simpler queries"
            )
    ...
    def _generate_strategy_recommendations(
        ...
        fastest_strategy = min(
            performance.items(), key=lambda x: x[1].get("took_ms", float("inf"))
        )
        slowest_strategy = max(
            performance.items(), key=lambda x: x[1].get("took_ms", 0)
        )
