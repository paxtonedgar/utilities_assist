...
class PerformanceMonitor:
    ...
    def get_metrics_summary(
        ...
        with self._lock:
            ...
            if search_strategy:
                filtered_metrics = [m for m in filtered_metrics if m.search_strategy == search_strategy]
    ...
    def get_performance_trends(
        ...
        with self._lock:
            ...
            # Calculate trend
            if len(buckets) >= 2:
                recent_avg = statistics.mean([b["mean"] for b in buckets[-3:]])  # Last 3 buckets
                earlier_avg = statistics.mean([b["mean"] for b in buckets[:3]])   # First 3 buckets
    ...
    def set_threshold(
        ...
        logger.info(f"Set threshold for {metric_name}: warning={warning_threshold}, critical={critical_threshold}")
    ...
    def _check_thresholds(self, metric: PerformanceMetric) -> None:
        ...
        recent_metrics = [
            m for m in list(self.metrics_history)[-threshold.window_size:]
