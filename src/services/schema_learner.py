...
class SchemaLearner:
    ...
    def observe_hit(self, index: str, hit: Dict, passages: List[Passage]) -> None:
        ...
        try:
            ...
            if passages:
                ...
                profile.content_paths[content_path] = (
                    profile.content_paths.get(content_path, 0) + 1
                )
            else:
                # Record failed extraction
                profile.content_paths["no_content"] = (
                    profile.content_paths.get("no_content", 0) + 1
                )
    ...
    def preferred_paths(self, index: str) -> List[str]:
        ...
        ranked_paths = sorted(
            profile.content_paths.items(), key=lambda kv: kv[1], reverse=True
        )
    ...
    def has_reliable_inner_hits(
        self, index: str, min_rate: float = 0.6, min_samples: int = 5
    ) -> bool:
        """
    ...
    def _get_top_content_path(self, profile: IndexProfile) -> str:
        ...
        valid_paths = {
            k: v for k, v in profile.content_paths.items() if k != "no_content"
        }
