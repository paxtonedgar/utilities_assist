...
try:
    ...
    logger.warning("Install with: pip install torch==2.2.2 sentence-transformers==3.0.1")
...
class CrossEncodeReranker:
    ...
    def score(self, query: str, passages: List[str]) -> List[float]:
        ...
        try:
            ...
            truncated_passages = [p[:50] + "..." if len(p) > 50 else p for p in passages[:3]]
            logger.debug(
    ...
    def rerank(
        ...
        if not docs:
            return [], RerankResult(
                scores=[], kept_indices=[], dropped_count=0,
                device_used=self.device, model_id=self.model_id, took_ms=0.0,
                top_scores=[], avg_score=0.0
            )
...
def get_reranker(
    ...
    if not RERANKER_AVAILABLE:
        logger.warning("Reranker not available - install torch and sentence-transformers")
        return None
