...
def extract_passages(hit: Dict[str, Any], cfg: ExtractorConfig) -> List[Passage]:
    ...
    return _postprocess(passages=passages, cfg=cfg)
        ...
        if isinstance(sections, list):
            for section in sections[:3]:  # Limit to first 3 sections
                if isinstance(section, dict):
                    ...
                    if text:
                        passage = _mk_passage(
                            hit, text, section.get("title") or section.get("heading")
                        )
                        passages.append(passage)
...
def _mk_passage(
    hit: Dict[str, Any], text: str, section_title: Optional[str]
) -> Passage:
    ...
    return Passage(
        ...
        meta={
            ...
            **{
                ...
                if k not in ["page_url", "api_name", "title"]
            },
        },
    )
...
def _postprocess(
    passages: List[Passage], cfg: ExtractorConfig
) -> List[Passage]:
