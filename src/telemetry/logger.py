...
ID_FIELD_MAPPINGS = {"reg_id": "req_id", "request_id": "req_id"}
...
def log_event(stage: str, req_id: Optional[str] = None, **kwargs):
    ...
    if reduce_verbosity:
        ...
        if event not in ["error", "success", "start"] and stage not in [
            "overall",
            "bm25",
            "knn",
        ]:
    ...
    # Build log data
    log_data = {"stage": stage, "req_id": req_id, "timestamp": time.time(), **kwargs}
...
def get_stage_logs(
    req_id: Optional[str] = None, last_n: int = 50
) -> List[Dict[str, Any]]:
...
def setup_logging():
    ...
    logger.info(
        f"Logging configured - structured: {settings.enable_structured_logging}, level: {settings.log_level}"
    )
