...
async def _create_batch_embeddings(
    embed_client: Any, texts: List[str], model: str
) -> List[List[float]]:
    """
    ...
    try:
        response = embed_client.embeddings.create(
            model=model,
            input=texts,
            dimensions=1536,  # Match Khub cluster mapping confirmed by colleague
        )
        ...
        for data_item in response.data:
            ...
            if not isinstance(embedding, list) or not all(
                ...
                raise EmbeddingError("Invalid embedding format received from API")
...
async def create_embeddings_with_retry(
    ...
    for i in range(0, len(text_list), batch_size):
        ...
        try:
            batch_embeddings = await _create_batch_embeddings(
                embed_client, batch, model
            )
    ...
    logger.info(
        f"Successfully created {len(all_embeddings)} embeddings with {expected_dims} dimensions"
    )
...
def validate_embedding_batch(
    ...
    for i, embedding in enumerate(embeddings):
        ...
        if len(embedding) != expected_dims:
            issues.append(
                f"Vector {i}: wrong dimensions ({len(embedding)} vs {expected_dims})"
            )

        # Check for NaN or infinite values
        if not all(
            isinstance(x, (int, float)) and not (x != x or abs(x) == float("inf"))
            for x in embedding
        ):
