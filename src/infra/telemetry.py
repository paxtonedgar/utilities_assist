...
def time_stage(stage: str, req_id: str, **fields):
    ...
    try:
        ...
        log_event(stage=stage, req_id=req_id, ms=round(dt, 2), error=err, **fields)
...
def log_intent_stage(
    req_id: str, intent: str, confidence: float, ms: float, error: Optional[str] = None
):
...
def log_verify_stage(
    req_id: str,
    verdict: str,
    unmatched_claims_count: int,
    ms: float,
...
def log_coverage_stage(
    req_id: str,
    gate_pass: bool,
    aspect_recall: float,
    alpha_ndcg: float,
    actionable_spans: int,
    selected_count: int,
    total_count: int,
...
def trace_retrieval(stage: str, req_id: str, k: int, **kwargs):
    ...
    try:
        ...
        result_count = (
            len(result.results) if result and hasattr(result, "results") else 0
        )
        top_ids = (
            [r.doc_id for r in result.results[:5]]
        ...
        if stage == "retrieve_bm25":
            ...
            log_retrieve_knn_stage(
                req_id, k, result_count, ms, kwargs.get("ef_search"), top_ids, error
            )
