...
def initialize_resources(
    settings: ApplicationSettings, force_refresh: bool = False
) -> RAGResources:
    """
    ...
    with _lock:
        if _resources is not None and not force_refresh:
            logger.info(
                f"Reusing existing resources (age: {_resources.get_age_seconds():.1f}s)"
            )
            return _resources
        ...
        if settings.cloud_profile == "jpmc_azure":
            ...
            logger.info(
                "Using Azure certificate authentication + API key (dual auth) for JPMC profile"
            )
        ...
        if (
            ...
            and settings.azure_openai.azure_openai_embedding_model
            and settings.embed
        ):
            embed_cfg = EmbedCfg(
                provider="azure",
            ...
            logger.warning(
                "Embed client not initialized - Azure OpenAI config or embed config missing"
            )
            logger.warning(
                f"azure_openai: {settings.azure_openai is not None}, embed: {settings.embed is not None}"
            )
...
def _load_config_once() -> dict:
    ...
    try:
            ...
            _cached_config_params.update(
                {
                    ...
                    "azure_openai_embedding_model": azure_section.get(
                        "azure_openai_embedding_model", ""
                    ),
                    "azure_openai_endpoint": azure_section.get(
        ...
        logger.info(
            f"Cached {len(_cached_config_params)} config parameters from config.ini"
        )
        return _cached_config_params
