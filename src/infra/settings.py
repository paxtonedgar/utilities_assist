...
class PerformanceBudgets(BaseModel):
    """Performance budget configuration for latency control."""

    slotting_ms_regex: int = Field(
        default=1,
        validation_alias="SLOT_REGEX_MS",
        description="Max time for regex-based intent slotting",
    )
    slotting_ms_onnx: int = Field(
        default=50,
        validation_alias="SLOT_ONNX_MS",
        description="Max time for ONNX intent classification",
    )
    ce_timeout_s: float = Field(
        default=1.8,
        validation_alias="CE_TIMEOUT_S",
        description="Cross-encoder timeout in seconds",
    )
    # Cache settings
    slot_ttl_s: int = Field(
        default=600,
        validation_alias="SLOT_TTL_S",
        description="Slot cache TTL in seconds",
    )
    ce_token_ttl_s: int = Field(
        default=300,
        validation_alias="CE_TOKEN_TTL_S",
        description="CE tokenization cache TTL in seconds",
    )
    slot_cache_size: int = Field(
        default=2048,
        validation_alias="SLOT_CACHE_SIZE",
        description="Maximum slot cache entries",
    )
    ce_token_cache_size: int = Field(
        default=8192,
        validation_alias="CE_TOKEN_CACHE_SIZE",
        description="Maximum CE token cache entries",
    )
    # Actionability thresholds
    procedure_score_threshold: float = Field(
        default=2.0,
        validation_alias="PROCEDURE_THRESHOLD",
        description="Actionability score threshold for procedure presenter",
    )
    min_actionable_spans: int = Field(
        default=2,
        validation_alias="MIN_ACTIONABLE_SPANS",
        description="Minimum actionable spans for procedure presenter",
    )
...
class ApplicationSettings(BaseSettings):
    ...
    def _load_simple_settings(
        self, config: configparser.ConfigParser, config_data: dict
    ):
        ...
        if config.has_section("file_paths"):
            ...
            config_data.update(
                {
                    "synonyms_file_path": section.get(
                        "synonyms_file_path", "data/synonyms.json"
                    ),
                    "api_file_path": section.get(
                        "api_file_path", "data/api_description.json"
                    ),
                    "questions_intent_path": section.get(
                        "questions_intent", "data/questions_intent.csv"
                    ),
                }
            )
    ...
    def requires_azure_auth(self) -> bool:
        ...
        return (
            self.cloud_profile.lower() == "jpmc_azure" and self.azure_openai is not None
        )
