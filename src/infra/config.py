...
def _jpmc() -> Settings:
    ...
    try:
        ...
        chat_deployment = os.getenv("AZURE_CHAT_DEPLOYMENT") or get_config_value(
            "azure_openai", "deployment_name", "gpt-4o-2024-08-06"
        )
        embed_deployment = os.getenv("AZURE_EMBED_DEPLOYMENT") or get_config_value(
            "azure_openai", "azure_openai_embedding_model", "text-embedding-3-small-1"
        )
        api_base = os.getenv("AZURE_OPENAI_ENDPOINT") or get_config_value(
            "azure_openai",
            "azure_openai_endpoint",
            "https://llm-multitenancy-exp.jpmchase.net/ver2/",
        )
        api_version = os.getenv("AZURE_API_VERSION") or get_config_value(
            "azure_openai", "api_version", "2024-10-21"
        )
        opensearch_host = os.getenv("OPENSEARCH_ENDPOINT") or get_config_value(
            "aws_info",
            "opensearch_endpoint",
            "https://utilitiesassist.dev.aws.jpmchase.net",
        )
        opensearch_index = os.getenv("OPENSEARCH_INDEX") or get_config_value(
            "aws_info", "index_name", OpenSearchConfig.get_default_index()
        )
        ...
        opensearch_index = os.getenv(
            "OPENSEARCH_INDEX", OpenSearchConfig.get_default_index()
        )
