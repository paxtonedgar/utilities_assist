...
class QueryAnalyzer:
    ...
    UTILITY_KEYWORDS = {
        'ciu': ['ciu', 'customer interaction utility', 'customer interaction', 'customer utility'],
    ...
    PROCEDURE_PATTERNS = [
        r'\bhow\s+to\b', r'\bsteps?\s+to\b', r'\bguide\s+(?:to|for)\b',
        r'\bonboard(?:ing)?\b', r'\bsetup?\b', r'\bconfigure?\b', r'\bintegrat\w+\b',
        r'\bimplement\w*\b', r'\binstall\w*\b', r'\benable\b', r'\bstart\w+\b'
    ]
    
    # API-focused indicators
    API_INDICATORS = [
        r'\bapi\b', r'\bendpoint\s*(?:s)?\b', r'\bswagger\b',
        r'\brest\b', r'\bhttp\b', r'\brequest\s*(?:s)?\b'
    ]
    ...
    def analyze_query(self, query: str) -> QueryCharacteristics:
        ...
        query_type = self._determine_query_type(
            query_lower, utility_specificity, generic_api_score, procedure_count, api_focus
        )
        ...
        reasoning = self._generate_reasoning(
            query_type, utility_specificity, generic_api_score, procedure_count, api_focus
        )
    ...
    def _determine_query_type(
        self, query_lower: str, utility_specificity: float, 
        generic_api_score: int, procedure_count: int, api_focus: bool
    ) -> QueryType:
        """Determine the primary query type."""
        
        # Check for troubleshooting
        if any(word in query_lower for word in ['error', 'fix', 'troubleshoot', 'not working', 'issue']):
    ...
    def _generate_reasoning(
        self, query_type: QueryType, utility_specificity: float,
        generic_api_score: int, procedure_count: int, api_focus: bool
    ) -> str:
...
def analyze_query_characteristics(query: str) -> QueryCharacteristics:
    """Convenience function to analyze query with global analyzer."""
    return _smart_threshold_selector.analyzer.analyze_query(query)